...
class MyPlayer:
    ...
    def get_player_action(self,
                        ...
                        turn,
                        game_index):
        # Obtain forecasts for the next spy value for both player and dealer,
        # and then map them to predicted card values.
        ...
        if turn == 'player':
            predicted_total = curr_player_total + next_pred_card_player
            
            # Adjust your aggressiveness based on the game index.
            # For example, early games (game_index < 50) use a higher threshold,
            # mid games (50<=game_index<150) a medium threshold, and late games (>=150) a lower threshold.
            if game_index < 50:
                threshold = 21  # more aggressive: willing to risk hitting up to 21
            elif game_index < 150:
                threshold = 20  # moderate
            else:
                threshold = 19  # conservative: avoid busting
            
            # If the predicted total exceeds 21, you should stand.
            if predicted_total > 21:
                return 'stand'
            # Otherwise, if the predicted total meets or exceeds your threshold, stand.
            elif predicted_total >= threshold:
                return 'stand'
            else:
                return 'hit'
        else:  # dealer's turn
            predicted_total = curr_dealer_total + next_pred_card_dealer
            # For the dealer, we use a simple rule: if the dealer's total is low (< 17) continue drawing.
            # Otherwise, if the dealer's total is already above the player's total, we choose to surrender to minimize losses.
            if predicted_total >= 17:
